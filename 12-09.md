...
(1 row)
```
Просмотр созданных баз данных (db1):
```sql
\l
```
Вывод:
...
##### Подключение к базе данных:
```
psql "host=rc1a-i1ouenefuwyr9ayq.mdb.yandexcloud.net,rc1b-q2cf8js354fp6oig.mdb.yandexcloud.net \
    port=6432 \
    sslmode=verify-full \
    dbname=db1_terraform \
    user=user1 \
    target_session_attrs=read-write"
```
Проверка подключения:
...
Вывод:
```sql
db1_terraform=> SELECT version();
                                                                     version                                                                     
-------------------------------------------------------------------------------------------------------------------------------------------------
 PostgreSQL 15.8 (Ubuntu 15.8-201-yandex.55089.7504eb5367) on x86_64-pc-linux-gnu, compiled by gcc (Ubuntu 11.4.0-1ubuntu1~22.04) 11.4.0, 64-bit
(1 row)
```
```sql
db1_terraform-> \l
     ...
     Name      |  Owner   | Encoding |   Collate   |    Ctype    |   Access privileges   
---------------+----------+----------+-------------+-------------+-----------------------
 db1_terraform | user1    | UTF8     | en_US.UTF-8 | en_US.UTF-8 | =T/user1             +
               |          |          |             |             | user1=CTc/user1      +
               |          |          |             |             | admin=c/user1        +
               |          |          |             |             | mdb_odyssey=c/user1  +
               |          |          |             |             | monitor=c/user1      +
